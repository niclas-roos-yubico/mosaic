...
-   `--cache-ttl <duration>`: Time-to-live for cache entries as a Go duration. 0s means no expiration (e.g., '10m', '1h'). Defaults to 0s.
    In this build it is not a freshness or authorization control once `--disable-result-cache=true` — see **Platform Security Mode**.
...
-   `--function-allowlist`: Comma-separated list of exact function names to add to the reviewed defaults. Names are matched case-insensitively, repeated flags accumulate names, and an explicitly empty value enables only the defaults.
    This build always applies the reviewed defaults regardless of this flag — see **Platform Security Mode**.
...
That first sentence does not apply to this build: the `duckdb-server-go` binary always calls
`WithFunctionAllowlist`, so the reviewed defaults are active even when `--function-allowlist` is never passed —
see **Platform Security Mode** above. Only a server built directly on `pkg/query` is unrestricted by default.
...
> **Fork note.** The section below is upstream's, restored verbatim for reference. It does not describe this
> build: `--schema-match-headers` is rejected at startup here, and schemas are derived from the validated
> session JWT instead — see **Platform Security Mode** above. Read it as documentation of the upstream
> mechanism this fork replaces, not as configuration advice for this binary.
...
This fork requires the `duckdb_arrow` build tag, which that line omits: build with
`go build -tags=duckdb_arrow -o duckdb-server-go .`, and test with `go test -tags=duckdb_arrow ./...` — the same
tag CI uses.
