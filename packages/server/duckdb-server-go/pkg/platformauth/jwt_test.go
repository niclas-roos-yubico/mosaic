...
func TestValidToken(t *testing.T) {
	...
	if claims.ExpiresAt.IsZero() {
		t.Fatal("expected validated exp in SessionClaims")
	}
	if time.Until(claims.ExpiresAt) < 50*time.Minute {
		t.Fatalf("ExpiresAt = %s, want approximately one hour from now", claims.ExpiresAt)
	}
...
func TestBadSignature(t *testing.T) {
	...
	v, vErr := platformauth.NewJWTValidator(platformauth.JWTValidatorConfig{
		JWKSURL:    srv.URL,
		Issuer:     "https://platform.example.com/platform",
		Audience:   "platform-data-plane",
		Algorithms: []string{"RS256"},
...
func TestEmptyIssuerRejectedAtConstruction(t *testing.T) {
	_, err := platformauth.NewJWTValidator(platformauth.JWTValidatorConfig{
...
func TestMissingExpirationClaim(t *testing.T) {
	priv, jwksHandler := genTestKey(t)
	srv := httptest.NewServer(jwksHandler)
	defer srv.Close()
	v, err := platformauth.NewJWTValidator(platformauth.JWTValidatorConfig{
	...
	_ = tok.Set(jwt.IssuedAtKey, now)
	_ = tok.Set(jwt.NotBeforeKey, now)
	_ = tok.Set(jwt.JwtIDKey, "jti-no-exp")
	_ = tok.Set("allowed_schemas", []string{"bookings"})
	key, err := jwk.Import(priv)
	if err != nil {
		t.Fatal(err)
	}
	_ = key.Set(jwk.KeyIDKey, "test-kid-1")
